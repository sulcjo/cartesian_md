...
vars=[33, 36, 41, 49, 68]
var=49
sec_files = pd.read_excel(f'/home/sulcjo/Desktop/49_2_tereza.ods')
empty_rows = 3
def clean_list(list):
    new_list = []
...
for column in sec_files.iteritems():
    ...
    if type(column) == list:
        if any('10_UV' in str(s) for s in column) and any('min' in str(s) for s in column):
            list_vol = column
            print(list_vol)
        ...
        elif any('_Fractions' in str(s) for s in column) and any('min' in str(s) for s in column):
            list_fravol = column
...
axs.set()
axs.set(xlim=(7,42))
axs.set_xlabel('t / min', size=16)
axs.set_ylabel('A / mAU', size=16)
axs.set_title(f'Var. {var}', size=20)
...
for x, label in zip(vlines_x, vlines_labels):
    ...
    plt.annotate(label, # this is the text
                 ...
                 size=16)
    even_odd += 1
plt.savefig(f'/home/sulcjo/Desktop/terkaakta/kl{var}.png')
plt.show()
