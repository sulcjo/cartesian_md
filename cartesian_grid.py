...
from cartesian_diff import color_structure
...
def main(argv=sys.argv[1:]):
    ...
    #parser.add_argument('--o', type=str, help='Output .csv file', required=False, default='grids.csv')
    ...
    if args.method == 'grid':
        ...
        if vectors2:
            ...
            print('No second vector-set --s provided, so no comparison will be done')
            exit()
        ...
        if args.pdbs:
            # Color .pdb by rms, takes in argument delta, which takes a pandas dataframe as such
            # "atom", "rms"
            delta = pd.Series(rms)
            ...
            if not args.s:
                print(
                    'Comparing structures requires two of them, please provide one more structure using the --s flag, will quit.')
                exit()
    ...
    if args.method == 'grid_scan':
        ...
        for gridsize in [3, 2, 1.5, 1, 0.5, 0.25, 0.1]:
            args.grid = gridsize
            atomic_grid = grid(vectors1)
            atomic_grid_uniq, grid_count = return_unique(atomic_grid)
